...
class SearchServer {
  constructor() {
    this.radio = new Emitter();
    this.sites = [new PirateBay(this.radio)];
  }
   ...
   * @param {Number} [num]
   */
  search(id, searchTerm, num = 30) {
    this.radio.emit('search', {
      id,
      searchTerm,
      num,
    });
  ...
  set radio(radio) {
    if(!this._radio) this._radio = radio;
    else throw new Error('Cannot set radio!');
  }
